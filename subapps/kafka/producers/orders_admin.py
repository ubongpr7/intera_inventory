...
def serialize_purchase_order(order: PurchaseOrder) -> dict[str, Any]:
    return {
        ...
        "issue_date": order.issue_date.isoformat() if order.issue_date else "",
        "delivery_date": order.delivery_date.isoformat() if order.delivery_date else "",
        "received_date": order.received_date.isoformat() if order.received_date else "",
...
def serialize_purchase_order_line_item(line_item: PurchaseOrderLineItem) -> dict[str, Any]:
    ...
    return {
        ...
        "expiry_date": line_item.expiry_date.isoformat() if line_item.expiry_date else "",
        "manufactured_date": line_item.manufactured_date.isoformat() if line_item.manufactured_date else "",
...
def serialize_goods_receipt(goods_receipt: GoodsReceipt) -> dict[str, Any]:
    return {
        ...
        "received_at": goods_receipt.received_at.isoformat() if goods_receipt.received_at else "",
...
def serialize_goods_receipt_line(goods_receipt_line: GoodsReceiptLine) -> dict[str, Any]:
    ...
    return {
        ...
        "manufactured_date": goods_receipt_line.manufactured_date.isoformat() if goods_receipt_line.manufactured_date else "",
        "expiry_date": goods_receipt_line.expiry_date.isoformat() if goods_receipt_line.expiry_date else "",
...
def serialize_sales_order(order: SalesOrder) -> dict[str, Any]:
    return {
        ...
        "notes": order.notes or "",
        "issue_date": order.issue_date.isoformat() if order.issue_date else "",
        "shipment_date": order.shipment_date.isoformat() if order.shipment_date else "",
        "delivery_date": order.delivery_date.isoformat() if order.delivery_date else "",
        "complete_date": order.complete_date.isoformat() if order.complete_date else "",
        "total_price": _json_decimal(order.total_price),
        "created_by_user_id": order.created_by_user_id,
        "updated_by_user_id": order.updated_by_user_id,
...
def serialize_sales_order_shipment(shipment: SalesOrderShipment) -> dict[str, Any]:
    return {
        ...
        "delivery_date": shipment.delivery_date.isoformat() if shipment.delivery_date else "",
...
def serialize_return_order(return_order: ReturnOrder) -> dict[str, Any]:
    return {
        ...
        "issue_date": return_order.issue_date.isoformat() if return_order.issue_date else "",
        "complete_date": return_order.complete_date.isoformat() if return_order.complete_date else "",
